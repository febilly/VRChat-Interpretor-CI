...
- 支持全局快捷键
    - 快速开关翻译器（默认 `Ctrl+Shift+T`）
    - 快速切换目标语言（默认 `Ctrl+Shift+L`）
    - 无需切换窗口，在游戏中即可操作
...
LANGUAGE_CYCLE = ['en', 'zh-CN', 'ja', 'ko']  # 可切换的目标语言列表
# 按顺序循环切换
# 可自定义语言和顺序
```

**快捷键功能说明：**
- **切换翻译器开关** (`Ctrl+Shift+T`): 快速启用或禁用语音识别和翻译功能
- **切换目标语言** (`Ctrl+Shift+L`): 在预设的语言列表中循环切换目标语言，切换时会在 VRChat 聊天框显示当前选择的语言
